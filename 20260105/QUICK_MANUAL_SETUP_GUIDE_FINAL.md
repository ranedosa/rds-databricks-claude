# Quick Manual Setup Guide - Census Syncs (FINAL - With Snappt_Property_ID__c as External ID)

**Time:** 30-45 minutes total
**Salesforce Change:** Snappt_Property_ID__c is now marked as External ID ✅

---

## Step 1: Wait for Census to Refresh (5-10 minutes)

After the Salesforce admin marks `Snappt_Property_ID__c` as an External ID, Census needs time to refresh its schema.

**Check if ready:**
1. Go to https://app.getcensus.com/syncs
2. Click "New Sync"
3. Select DBX (Databricks) → `crm.sfdc_dbx.properties_to_create`
4. Select Salesforce Production → Product_Property__c
5. Check if `Snappt_Property_ID__c` appears in the Sync Key dropdown

If not visible yet, wait 5 more minutes and try again.

---

## Step 2: Create Sync A (CREATE) - 15 minutes
...
- **Sync Key:** Snappt_Property_ID__c (should now be visible!)

### Field Mappings (19 fields)

**Clean and simple - no duplicate mappings needed:**

```
rds_property_id → Snappt_Property_ID__c (Primary Key - CHECK THIS)
...
## Step 3: Create Sync B (UPDATE) - 15 minutes
...
### Sync Behavior
- **Operation:** Update (or Upsert if you want it to create records that don't exist)
- **Sync Key:** Snappt_Property_ID__c

### Field Mappings (19 fields - same as Sync A)

```
snappt_property_id_c → Snappt_Property_ID__c (Primary Key - CHECK THIS)
short_id → Short_ID__c
company_id → Company_ID__c
property_name → Name
address → Address__Street__s
...
**Note:** For UPDATE sync, we map `snappt_property_id_c` (the existing SF value from the source view) to match records.
...
Enter the two Sync IDs when prompted.

---

## Step 5: Run Pilot Tests via API
...
## Step 6: Validate Results in Salesforce

1. Go to Salesforce → Product Properties
2. Search for a few test property IDs from the pilot list
3. Verify:
   - ✅ New properties were created (from Sync A)
   - ✅ Existing properties were updated (from Sync B)
   - ✅ Feature flags match Databricks source data
...
**If successful → GO for Day 3 full rollout**
**If errors > 10% → STOP, debug, and retry**

---

## Troubleshooting

### Snappt_Property_ID__c still not showing
- Wait another 5 minutes
- Hard refresh Census page (Cmd+Shift+R or Ctrl+Shift+R)
- Try clicking "Refresh Schema" if available in Census
- Contact Census support if still not visible after 15 minutes

### Field mapping errors
- Ensure field names exactly match (case-sensitive)
- Use Address__Street__s not Address__street__s
- Use ID_Verification_Enabled__c not Id_Verification_Enabled__c

### Pilot filter errors
- Ensure single quotes around UUIDs
- Ensure column name has no quotes: `rds_property_id IN ('...')`

---

**Current Status:** Ready for clean sync creation with Snappt_Property_ID__c

**Next Action:**
1. Wait 5-10 min for Census to refresh
2. Verify Snappt_Property_ID__c appears as sync key option
3. Create both syncs following this guide
