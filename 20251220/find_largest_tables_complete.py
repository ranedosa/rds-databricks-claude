#!/usr/bin/env python3
"""
Complete analysis of largest tables - ALL catalogs except exclusions
"""
...
def get_table_size(full_table_name):
    """Get table size using SELECT COUNT"""
...
def main():
    print("=" * 80)
    print("Complete Analysis: ALL Catalogs (Except Exclusions)")
    print("=" * 80)

    # Get all managed/external Delta tables - NO catalog filter, only exclusions
    query = """
    SELECT
        t.table_catalog,
        t.table_schema,
    ...
    WHERE t.table_type IN ('MANAGED', 'EXTERNAL')
      ...
      AND t.table_catalog NOT LIKE '%__databricks_internal%'
      AND t.table_name NOT LIKE '%_fivetran%'
    ORDER BY t.table_catalog, t.table_schema, t.table_name
    """

    log("Getting complete list of tables across ALL catalogs")
    ...
    catalog_counts = tables_df.groupby('table_catalog').size().sort_values(ascending=False)
    print("\nTables by catalog:")
    ...
    log("Starting row count analysis...")
    ...
    if all_stats:
        ...
        # Save results
        output_file = '/Users/danerosa/rds_databricks_claude/20251220/largest_tables_complete_top100.csv'
        top_100.to_csv(output_file, index=False)
        log(f"Saved to: {output_file}", "SUCCESS")

        # Full results
        full_output = '/Users/danerosa/rds_databricks_claude/20251220/all_table_row_counts_complete.csv'
        ...
        print("SUMMARY")
        print("=" * 80)
        total_rows = int(top_100['row_count'].sum())
        avg_rows = int(top_100['row_count'].mean())
        median_rows = int(top_100['row_count'].median())
        largest_rows = int(top_100.iloc[0]['row_count'])

        print(f"Total rows (top 100): {total_rows:,}")
        print(f"Average rows (top 100): {avg_rows:,}")
        print(f"Median rows (top 100): {median_rows:,}")
        print(f"Largest: {top_100.iloc[0]['full_table_name']} ({largest_rows:,} rows)")
